...
class Causal: PostgresTable, PostgresJson {
    ...
    func setupDefaults() throws {
        let rows: [Causal] = try self.query(cursor: Cursor(limit: 1, offset: 0)).wait()
        if rows.count == 0 {
